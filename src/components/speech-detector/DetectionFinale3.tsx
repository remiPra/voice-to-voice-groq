...
À chaque message, tu t'exprimes en moins de 80 mots , chaleureuses et encourageantes, qui réchauffent le cœur`;
  ...
  const handleImageUpload = async (
    ...
    if (file) {
      try {
        ...
        setIsImageMode(true);
        console.log("Image encodée et prête pour l'analyse");
      } catch (error) {
  ...
  const handleMessageSubmission = async (content: string) => {
    ...
    try {
      ...
      // Pour l'historique, on ajoute juste le texte
      const historyMessage = { role: "user", content };
      ...
      let apiMessages = [...messageHistory.current];
      let shouldUseImage = false;

      // Si c'est le premier tour avec image, on ajoute l'image
      if (isImageMode && currentImage) {
        apiMessages.push({
          role: "user",
          content: [
        ...
        // Désactiver le mode image après ce premier envoi
        setIsImageMode(false);
      } else {
        // Pas d'image, juste le texte
      ...
      if (shouldUseImage) {
        // API Mistral avec le modèle pixtral pour les images
        ...
        // API Groq avec gemma2 pour le texte seulement
      ...
      if (data.choices?.[0]?.message?.content) {
        ...
        messageHistory.current = [
          ...messageHistory.current,
          historyMessage, // Le message utilisateur (texte uniquement)
          ...
          >
            <div className="flex items-center">
              {isImageMode && currentImage && (
                <div className="fixed top-20 right-4 bg-[#ff9000] text-white px-4 py-2 rounded-lg shadow-lg z-50">
                  Mode analyse d'image activé 📷
                </div>
              )}
              <input
                type="text"
              ...
              <div className="p-5 border-b border-gray-200">
                ...
                <div className="p-5 border-b border-gray-200">
                  ...
                  <button
                    ...
                    className="w-full px-4 py-2.5 rounded-md font-medium bg-[#ff9000] hover:bg-[#e67e00] text-white transition-all duration-300 shadow-md mb-2"
                  >
                    Charger une image
                  </button>
                  ...
                  {currentImage && (
                    <>
                      <div className="mb-2 p-2 bg-green-100 text-green-700 rounded">
                        Image chargée - Mode analyse d'image activé
                      </div>
                      <button
                        onClick={() => {
                        ...
                        className="w-full px-4 py-2.5 rounded-md font-medium bg-[#e63946] hover:bg-[#cc2936] text-white transition-all duration-300 shadow-md"
                      >
                        Désactiver l'analyse d'image
                      </button>
                    </>
                  )}
                </div>
