...
const SpeechDetector: React.FC<SpeechDetectorProps> = ({
  ...
  const finishCalibration = () => {
    if (noiseFloorRef.current.length > 0) {
      ...
      const variance = noiseFloorRef.current.reduce((a, b) => a + Math.pow(b - mean, 2), 0) / noiseFloorRef.current.length;
      ...
      console.log(`Calibration terminée. Nouveau seuil: ${newThreshold.toFixed(4)}`);
    }
...
//   };
  ...
  const saveRecording = async (audioBlob: Blob) => {
    ...
    audio.onloadedmetadata = async () => {
      ...
      console.warn("⏳ Ignoré: Transcription non significative:", transcriptionText);
      return;
    }
  ...
  const smoothVolume = (newVolume: number): number => {
    ...
    const smoothedValue = (alpha * newVolume) + ((1 - alpha) * volumeHistory.current[volumeHistory.current.length - 1]);
  ...
  const startListening = async () => {
    try {
      ...
      await audioContextRef.current.audioWorklet.addModule(
        URL.createObjectURL(
          new Blob([`registerProcessor("${vuWorkletName}", ${VolMeterWorket})`], { type: "application/javascript" })
        )
      );
      ...
      vuWorkletRef.current.port.onmessage = (ev: MessageEvent) => {
        ...
        if (analyserRef.current && frequencyDataRef.current) {
          ...
          const voiceFrequencyData = Array.from(frequencyDataRef.current.slice(3, 25));
          ...
          if (smoothedVolume > currentThreshold && hasVoiceFrequency) {
            ...
            if (!isSpeaking) {
              if (!speechStartTimeRef.current) {
                ...
                const validationDelay = firstSpeechDetectedRef.current ? 150 : 200;
            ...
            if (speechBooleanStateRef.current === 1 && silenceCountRef.current > silenceThreshold) {
              ...
              if (!graceTimeoutRef.current) {
                ...
                }, 500); // 1 seconde de grâce pour les pauses
              }
            }
          }
        }
           ...
           >
            ...
            <div className="p-4 border-b">
              ...
              {isCalibrating && (
                <div className="w-full bg-gray-200 rounded-full h-2.5 mt-2">
                  ...
                  ></div>
                  <p className="text-xs text-gray-500 mt-1">Calibration: {calibrationProgress.toFixed(0)}%</p>
                </div>
              )}
            </div>
            ...
            <div className="p-4 border-b">
              ...
              {transcriptions.length === 0 ? (
                <p className="text-gray-500 italic text-sm">Aucune transcription pour le moment</p>
              ) : (
                <div className="space-y-2 max-h-40 overflow-y-auto">
                  {transcriptions.map((trans) => (
                    <div key={trans.id} className="p-2 bg-gray-50 border rounded text-sm">
                      <div className="flex justify-between mb-1">
                        <span className="text-xs font-medium">Transcription</span>
                        <span className="text-xs text-gray-500">{trans.timestamp}</span>
                      </div>
                      <p>{trans.text}</p>
                    </div>
                  ))}
            ...
            <div className="p-4 border-b">
              ...
              {audioUrls.length === 0 ? (
                <p className="text-gray-500 italic text-sm">Aucun audio généré pour le moment</p>
              ) : (
            ...
            <div className="p-4 border-b">
              <h3 className="text-md font-semibold mb-2">Informations de débogage</h3>
              <div className="text-xs space-y-1 bg-gray-50 p-2 rounded">
                <p>Volume actuel: {volume.toFixed(5)}</p>
                <p>Seuil actuel: {threshold.toFixed(5)}</p>
                <p>Seuil après première détection: {(threshold * 0.8).toFixed(5)}</p>
                <p>Première parole détectée: {firstSpeechDetectedRef.current ? "Oui" : "Non"}</p>
