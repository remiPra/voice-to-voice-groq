...
const DetectionFinal2: React.FC<SpeechDetectorProps> = ({
  ...
  // États pour la détection d'interruption par mot-clé
  ...
  // Refs pour la détection de mot-clé
  const keywordRecorderRef = useRef<MediaRecorder | null>(null);
  const keywordAudioChunksRef = useRef<Blob[]>([]);
  const keywordCheckIntervalRef = useRef<number | null>(null);
  const KEYWORD_STOP = "stop";
  const KEYWORD_END_SESSION = "fin de session";
  ...
  // Fonction pour démarrer la détection de mot-clé
  const startKeywordDetection = () => {
    if (!streamRef.current || !isTTSAudioPlayingRef.current) return;
    
    console.log("🎤 Démarrage de la détection des mots-clés", KEYWORD_STOP, "et", KEYWORD_END_SESSION);
    
    // Arrêter toute détection en cours
    stopKeywordDetection();
    
    // Démarrer un nouvel enregistrement
    keywordAudioChunksRef.current = [];
    const options = { mimeType: "audio/webm" };
    
    try {
      keywordRecorderRef.current = new MediaRecorder(streamRef.current, options);
      keywordRecorderRef.current.ondataavailable = (event) => {
        if (event.data.size > 0) {
          keywordAudioChunksRef.current.push(event.data);
        }
      };
      
      // Configurer l'intervalle d'analyse toutes les 2 secondes
      keywordRecorderRef.current.start(2000); // Génère des chunks toutes les 2s
      
      // Traiter les données audio pour la transcription et vérifier le mot-clé
      keywordRecorderRef.current.onstop = async () => {
        // Ne rien faire si l'audio TTS n'est plus en cours
        if (!isTTSAudioPlayingRef.current) return;
        
        if (keywordAudioChunksRef.current.length > 0) {
          const audioBlob = new Blob(keywordAudioChunksRef.current, { type: "audio/webm" });
          
          // Transcription de l'audio
          console.log("🔍 Vérification de la présence des mots-clés...");
          const transcription = await sendAudioForTranscription(audioBlob);
          
          if (transcription && transcription.text) {
            const transcriptionText = transcription.text.toLowerCase().trim();
            console.log("📝 Transcription obtenue:", transcriptionText);
            
            // Vérifier si le mot-clé STOP est présent
            if (transcriptionText.includes(KEYWORD_STOP)) {
              console.log(`🛑 MOT-CLÉ "${KEYWORD_STOP}" DÉTECTÉ! Interruption en cours...`);
            ...
            // Vérifier si la commande FIN DE SESSION est présente
            if (transcriptionText.includes(KEYWORD_END_SESSION)) {
              console.log(`🔚 COMMANDE "${KEYWORD_END_SESSION}" DÉTECTÉE! Arrêt de l'écoute...`);
              
              // Afficher un message dans les transcriptions
              setTranscriptions((prev:any) => [
                ...prev,
                {
                  id: `command-${Date.now()}`,
                  text: `Commande '${KEYWORD_END_SESSION}' détectée. Écoute désactivée.`,
        ...
        // Continuer l'enregistrement si TTS est toujours en cours
        if (isTTSAudioPlayingRef.current && streamRef.current && !keywordDetected) {
          startKeywordDetection();
        }
      };
      
    } catch (err) {
      console.error("Erreur lors du démarrage de la détection de mot-clé:", err);
    }
  };

  const stopKeywordDetection = () => {
    if (keywordRecorderRef.current && keywordRecorderRef.current.state !== "inactive") {
      keywordRecorderRef.current.stop();
    }
  ...
  // Effet pour gérer l'interruption
  useEffect(() => {
  ...
  const speakResponse = async (text: string) => {
    ...
    try {
      ...
      // Créer une référence directe à l'élément audio
      const audioElement = new Audio(audioUrl);
      audioElement.playbackRate = playbackRate;

      // Variable globale pour accéder à l'audio en cours de lecture
      ...
      audioElement.onplay = () => {
        ...
        // Démarrer la détection de mot-clé
        startKeywordDetection();
      };

      // Configurer le callback de fin de lecture
      audioElement.onended = () => {
        // Arrêter la détection de mot-clé
        stopKeywordDetection();
  ...
  const saveRecording = async (audioBlob: Blob) => {
    ...
    audio.onloadedmetadata = async () => {
      ...
      if (transcription && transcription.text) {
        ...
        if (transcriptionText.toLowerCase().includes(KEYWORD_END_SESSION)) {
          console.log(`🔚 Commande '${KEYWORD_END_SESSION}' détectée - arrêt de l'écoute`);
          
          // Ajouter un message visuel pour l'utilisateur
          setTranscriptions((prev:any) => [
            ...prev,
            {
              id: `command-${Date.now()}`,
         ...
         >
           ...
           <div className="p-5 border-b border-gray-200">
             ...
             <div className="text-xs space-y-2 bg-gray-100 p-3 rounded-lg text-gray-700">
               ...
               <p>Détection basée sur: Mots-clés "stop" et "fin de session"</p>
               <p>
                 Interruption actuelle:{" "}
                 {keywordDetected ? `Mot-clé "${KEYWORD_STOP}" détecté` : "Aucune"}
               </p>
             </div>
           </div>
           <div className="p-5">
